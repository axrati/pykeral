...
def node_df_config_linter(config):
    ...
    if mandatory_root_key_check==False:
        raise Exception("A mandatory key in the node config input is not there. Mandatory keys are: \n{}".format(str(mandatory_root_keys)))
    if available_root_key_check==False:
        raise Exception("A key in the node input is not a valid key. Availabled keys are: \n{}".format(str(available_root_keys)))
    ...
    if derived_check:
        for derv in config['derived']:
            ...
            if mandatory_root_key_check == False:
                raise Exception("A key was provided to the node derived input that shouldnt be there. Available keys are:\n{}".format(str(mandatory_root_keys)))
            columns_dict_check = type(derv['columns'])==list
            if columns_dict_check == False:
                raise Exception("The columns argument was not provided a list for a node derived value, please supply an array of column strings or indexes:\n")
